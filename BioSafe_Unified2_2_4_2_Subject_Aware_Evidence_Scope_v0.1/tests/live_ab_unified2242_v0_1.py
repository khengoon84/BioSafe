...
A="http://127.0.0.1:8774";B="http://127.0.0.1:8775"
...
for name,q in cases:
 ...
 tb=visible(b);print(f"\n== {name} ==\nA(8774): {visible(a)[:1600]}\nB(8775): {tb[:1600]}")
 checks=[("http",ca==200 and cb==200),("B_nonempty",bool(tb))]
 if name=="definition":checks+=[("definition_clean",not any(x in tb.lower() for x in ("document review is advisory","current malaysian","act 678","ibc assessment report")))]
 ...
 ("no_forme_pollution","assessment report" not in tb.lower())]
...
print(f"\nUnified-2.2.4.2 A/B hard assertions: {P}/{T} PASS")
