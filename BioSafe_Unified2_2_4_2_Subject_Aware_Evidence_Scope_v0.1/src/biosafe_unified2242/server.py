...
from biosafe_unified2242.service import Unified2242Service
app=Flask(__name__);orch=UnifiedOrchestrator();svc=Unified2242Service()
@app.get("/health")
def health():return jsonify({"status":"ok","stage":"Unified-2.2.4.2 v0.1","port":8775,
"subject_aware_scope":True,"frozen_core_modified":False})
@app.post("/api/ask")
def ask():
 ...
 return jsonify(out)
if __name__=="__main__":app.run(host="127.0.0.1",port=8775,debug=False)
