...
function quarantineLegacySiblings(app,host){
  [...document.body.children].forEach(node=>{
    if(node===app || node===host) return;
    if(node.tagName==="SCRIPT" || node.tagName==="STYLE" || node.tagName==="LINK") return;
    node.setAttribute("data-biosafe-legacy-hidden","true");
...
function renderOutputMirror(raw){
  ...
  if(obj){
    const conclusion=(obj.direct_answer||obj.conclusion||"").trim();
...
function clearVisibleEditor(){
  const editor=document.querySelector(".biosafe-chat-editor");
  if(editor) editor.value="";
}
function clearHiddenSourceLater(mode){
  setTimeout(()=>{
    const src=q(mode);
    if(src) src.value="";
  },0);
...
function submitActive(){
  ...
  const text=q(modeAtSubmit)?.value||"";
  ...
  const b=originalButton(modeAtSubmit);
  if(b){
    // Preserve the hidden Stage 9 textarea value until the legacy onclick handler has consumed it.
    b.click();
  }

  clearVisibleEditor();
  clearHiddenSourceLater(modeAtSubmit);
}
...
function mirrorOutput(){
  ...
  if(!raw || raw==="No request submitted yet.")return;
  // Repeated identical responses are valid separate conversation turns.
...
function build(){
  ...
  document.documentElement.classList.add("biosafe-chat-active");
  ...
  quarantineLegacySiblings(app,host);
