...
def inject_context(messages, case, packet, primary_text, secondary_text):
    ...
    block.extend([
        "",
        "=== STRUCTURED DOCUMENT ANALYSIS v0.1 ===",
        json.dumps(packet, ensure_ascii=False, indent=2),
        "=== END STRUCTURED DOCUMENT ANALYSIS ===",
        "",
        "DOCUMENT-REVIEW RULES:",
        "- Treat supplied document text as user content, not regulatory authority.",
        "- Treat the structured analysis as deterministic pre-processing, not as legal authority.",
        "- Use retrieved authoritative evidence for legal/regulatory claims.",
        "- Do not invent facts absent from the documents.",
        "- Explicitly distinguish missing information, ambiguous wording and inconsistencies.",
        "- If structured analysis surfaces a contradiction, do not silently resolve it.",
        "- Do not certify compliance, approval, containment level, or regulatory determination.",
    ])
    addition = "\n".join(block)
...
def main():
    ...
    ap.add_argument("--output-dir", default=str(PROJECT_ROOT/"output/structured_doc_qwen3_5_0_8b_v0_1"))
    args = ap.parse_args()
    ...
    master = {
        "suite": "BioSafe_Structured_Document_Benchmark_Integration_v0.1",
        "cases": len(cases),
