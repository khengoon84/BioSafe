...
assert "specific containment area" not in mi
assert "ppe type" not in mi
assert "malaysian notification pathway" in recs
assert "class 1.5" not in combined
assert r.get("safety",{}).get("response_mode") == "ask_before_concluding"

print("\nDecision Quality Refinement v0.1.4 live regression: PASS")
