...
if template is None:
    raise SystemExit("Could not locate index.html under app/templates or templates. No files changed.")
...
for name in ["biosafe_response_renderer.css","biosafe_response_renderer.js"]:
    shutil.copy2(HERE/"assets"/name,assets_dir/name)

stamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
backup = template.with_suffix(f".html.bak_{stamp}")
shutil.copy2(template,backup)

html = template.read_text(encoding="utf-8")
css_tag = '<link rel="stylesheet" href="/static/biosafe_stage10/biosafe_response_renderer.css">'
js_tag = '<script defer src="/static/biosafe_stage10/biosafe_response_renderer.js"></script>'
...
print("BioSafe Stage 10.1 Response Presentation v0.1: INSTALLED")
print("Inference architecture modified: NO")
