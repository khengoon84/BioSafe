...
class Unified225Service:
    def __init__(self):
        self.planner=EvidenceRequirementPlanner(); self.scope=SubjectAwareEvidenceScopeEnforcer()
        self.constitution=load_constitution(); self.engine=frozen.BioSafeFullInferenceServiceV011(ROOT)
        self.contract=ResponseTypeContractEnforcer(); self.semantic=EvidencePreservingSemanticVerifier()
        self.decision=DecisionSemanticsGuard(); self.coverage=RequestedSubjectCoverageGuard()
    ...
    def infer(self,prep,documents=None):
        ...
        scoped=ScopedPipelineAdapter(original_pipeline,self.scope,plan,prep["query"]); captured={"evidence":[]}
        ...
        out,da=self.decision.apply(out,prep["query"],prep.get("case_state") or {},captured["evidence"])
        out,coa=self.coverage.apply(out,prep["query"],captured["evidence"])
        out,sra=self.safety_rationale.apply(out,prep["query"])
        out,ca=self.contract.apply(out,i)
        out["_normalized_intent"]=i; out["_evidence_plan"]=plan.__dict__; out["_scope_audit"]=scoped.audit
        out["_semantic_verifier"]={"audit":sa,"scoped_evidence_ids":[x.get("evidence_id") for x in captured["evidence"]]}
        out["_decision_semantics"]={"audit":da}; out["_subject_coverage"]={"audit":coa}
        out["_safety_rationale"]={"audit":sra}; out["_response_contract"]={"intent":i,"audit":ca}
        return out
