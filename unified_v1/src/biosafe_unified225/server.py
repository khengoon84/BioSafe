...
from biosafe_unified225.service import Unified225Service
app=Flask(__name__); orch=UnifiedOrchestrator(); svc=Unified225Service()
@app.get("/health")
def health(): return jsonify({"status":"ok","stage":"Unified-2.2.5 v0.1","port":8776,"frozen_core_modified":False})
@app.post("/api/ask")
def ask():
 ...
 return jsonify(out)
if __name__=="__main__":app.run(host="127.0.0.1",port=8776,debug=False)
