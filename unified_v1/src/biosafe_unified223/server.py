...
app=Flask(__name__);orch=UnifiedOrchestrator();svc=Unified223Service()
@app.get("/health")
def health(): return jsonify({"status":"ok","stage":"Unified-2.2.3 v0.1","port":8772,"frozen_core_modified":False})
@app.post("/api/ask")
def ask():
    ...
    out=svc.infer(prep,d.get("documents") or []);out["session_id"]=prep["session_id"];return jsonify(out)
if __name__=="__main__":app.run(host="127.0.0.1",port=8772,debug=False)
