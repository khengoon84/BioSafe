...
from biosafe_unified2241.service import Unified2241Service
app=Flask(__name__);orch=UnifiedOrchestrator();svc=Unified2241Service()
@app.get("/health")
def health():return jsonify({"status":"ok","stage":"Unified-2.2.4.1 v0.1","port":8774,
"evidence_preserving_semantic_repair":True,"frozen_core_modified":False})
@app.post("/api/ask")
def ask():
 ...
 return jsonify(out)
if __name__=="__main__":app.run(host="127.0.0.1",port=8774,debug=False)
