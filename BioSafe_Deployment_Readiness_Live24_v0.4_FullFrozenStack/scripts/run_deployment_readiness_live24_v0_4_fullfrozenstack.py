...
def doc_filename(doc):
    ext = {"sop":".txt", "proposal":".txt", "form_e":".txt"}.get(doc.get("type"), ".txt")
    return f"{doc['id']}{ext}"
...
def main():
    ...
    for i, case in enumerate(cases, 1):
        ...
        try:
            ...
            # Hard safety short-circuit from the actual policy guard.
            ...
            bundle, base_messages = pipeline.build_messages(case["query"], case_id=case["id"], safety_class=None)
            ...
            assembled = enforce_response_budget(
                assembled,
                max_missing=getattr(profile,"max_missing_findings",3),
                max_recommendations=getattr(profile,"max_recommendations",2),
                max_limitations=getattr(profile,"max_limitations",1),
    ...
    out = {
        "suite":"BioSafe_Deployment_Readiness_Live24_v0.4_FullFrozenStack",
    ...
    outfile = outdir/"deployment_readiness_live24_v0.4_fullfrozenstack_results.json"
