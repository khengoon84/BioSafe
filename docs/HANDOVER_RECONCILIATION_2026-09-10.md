...
- Canonical claim map `config/claim_reconciliation_map_v0_1.json` (SHA-256 `b7d3a901…`):
  45 claims, 24 completed (9 `CURRENTNESS_UNRESOLVED` + 8 Form E + 4 contained-use + 3 GMM risk-assessment guidance), 21 pending;
  all-pending baseline preserved byte-identically at `reports/claim_reconciliation_map_v0_1_pre_legal_review.json`
  (`1edf2c9a…`).
- `reports/claim_reconciliation_packet_v0_1.json` (`b20cf1ab…`): 45 required / 24 completed /
  12 curated (CLM-008–CLM-014 and CLM-021–CLM-025), `PROHIBITED_PENDING_PHASE_C_GATES`.
- `reports/curated_candidate_kb_v0_1.json` (`e95d07ca…`): 12 offline curated candidate claims.
- `reports/claim_reconciliation_review_aid_v0_1.json` (`2e125f56…`): navigation aid,
  24 complete / 21 pending across 6 controlled documents.
- All deterministic test suites in `controlled_sources/ingestion_v0_1/tests/` pass
  after the GMM risk-assessment guidance review application (run 11 September 2026).
...
3. Navigation-only briefs for all 21 pending claims now exist under
