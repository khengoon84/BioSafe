...
- Canonical claim map `config/claim_reconciliation_map_v0_1.json` (SHA-256 `bc52b440…`):
  45 claims, 42 completed (9 `CURRENTNESS_UNRESOLVED` + 8 Form E + 4 contained-use
  + 3 Malaysian GMM risk-assessment guidance + 3 Malaysian transport guidance + 3 IBC governance
  + 9 WHO LBM4 risk-assessment guidance + 3 WHO LBM4 core-manual claims), 3 pending;
  all-pending baseline preserved byte-identically at `reports/claim_reconciliation_map_v0_1_pre_legal_review.json`
  (`1edf2c9a…`).
- `reports/claim_reconciliation_packet_v0_1.json` (`5e75890b…`): 45 required / 42 completed /
  29 curated, `PROHIBITED_PENDING_PHASE_C_GATES`.
- `reports/curated_candidate_kb_v0_1.json` (`7fb422af…`): 29 offline curated candidate claims.
- `reports/claim_reconciliation_review_aid_v0_1.json` (`64311667…`): navigation aid,
  42 complete / 3 pending across 2 controlled documents.
- All deterministic test suites in `controlled_sources/ingestion_v0_1/tests/` pass
  after the WHO LBM4 core-manual review application (run 11 September 2026).
...
3. Navigation-only briefs for all 3 pending claims now exist under
