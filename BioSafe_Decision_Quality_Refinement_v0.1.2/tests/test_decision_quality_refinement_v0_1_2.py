...
def base():
    return {
        ...
        "missing_information": [
            ...
            "Specific BSL classification of the culture"
        ],
...
cases.append(("DQ-006-remove-ungrounded-bsl", "bsl classification" not in mi))
...
cases.append(("DQ-007-transport-kept-when-grounded", any("transport" in x.lower() for x in r2.response["missing_information"])))

r3=g.apply(
    {
        "conclusion":"The information supplied is insufficient to assess containment.",
        "missing_information":["Specific BSL classification of the culture"],
        "recommended_next_step":[],
        "limitations":[],
        "safety":{}
    },
    user_query="What biosafety level is used for this culture in our containment facility?"
)
cases.append(("DQ-008-bsl-kept-when-grounded", any("bsl" in x.lower() for x in r3.response["missing_information"])))
...
cases.append(("DQ-009-generic-answer-unchanged", not r4.changed))
...
print("Decision Quality Refinement v0.1.2 regression: PASS")
