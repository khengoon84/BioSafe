...
def main():
    ...
    master_summary = {
        "suite": "BioSafe_Qwen3.5_0.8B_50Case_Challenger_v0.2.2",
        "pipeline": {
            "knowledge_base": "BioSafe KB v0.2.2",
            "retrieval": "CFG-02",
            "authority_router": "Integration Authority Router v0.1",
            "query_router": "Query Router v0.2.2",
            "scope_gate": "Scope Gate v0.2.2.1",
                ...
                if deterministic is not None:
                    ...
                    try:
                        try:
                            ...
                            print(
                                f"Transient Ollama transport error for {case_id}: {exc!r}; retrying once.",
                                flush=True,
                            )
                            time.sleep(0.5)
                            gen = stream_chat(host, model, messages, cfg, timeout=args.timeout)
