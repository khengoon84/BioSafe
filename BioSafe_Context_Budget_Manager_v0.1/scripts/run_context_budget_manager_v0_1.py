...
MODELS = ["qwen3.5:0.8b"]
MODEL_CONFIGS = {
    "qwen3.5:0.8b": {"temperature": 0.0, "num_predict": 700, "think": False},
    "qwen3.5:2b": {"temperature": 0.0, "num_predict": 900, "think": False},
}
...
def main():
    ...
    ap.add_argument(
        "--regression-only",
        action="store_true",
        help="Run only PROP-002 final freeze-gate case."
    )
    ...
    ap.add_argument("--output-dir", default=str(PROJECT_ROOT/"output/context_budget_manager_qwen3_5_0_8b_v0_1"))
    args = ap.parse_args()
    ...
    master = {
        "suite": "BioSafe_Context_Budget_Manager_v0.1",
        "cases": len(cases),
        ...
        with outfile.open("w", encoding="utf-8") as f:
            for idx, case in enumerate(cases, 1):
                ...
                bundle = merge_document_evidence_into_bundle(
                    bundle, packet, max_items=profile.max_document_evidence
                )

                budgeted_evidence, evidence_budget_audit = budget_evidence(
                    bundle.get("evidence_bundle", []), profile
                )
                bundle["evidence_bundle"] = budgeted_evidence
                bundle["_model_id"] = model

                packet = compact_packet_for_budget(packet, profile)
                ...
                for mi in range(len(messages)-1, -1, -1):
                    if messages[mi].get("role") == "user":
                        ...
                        messages[mi]["content"] = (
                            messages[mi].get("content", "") + response_contract(profile)
                        )
                        break
                ...
                if deterministic is not None:
                    ...
                    contradiction_fallback_used = False
                    preliminary_schema_issue = False

                    if valid and isinstance(parsed0, dict) and packet.get("contradictions"):
                        try:
                            prelim_core = validate_output(
                                parsed0, bundle, valid_json=True
                            )
                            preliminary_schema_issue = (
                                not prelim_core.get("checks", {}).get("schema_valid", True)
                            )
                        except Exception:
                            preliminary_schema_issue = True

                    if (
                        (not valid or gen.get("done_reason") == "length" or preliminary_schema_issue)
                        and packet.get("contradictions")
                    ):
