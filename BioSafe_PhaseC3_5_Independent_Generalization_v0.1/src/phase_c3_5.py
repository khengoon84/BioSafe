...
def cases(kb: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for claim in sorted(kb["claims"], key=lambda x:x["claim_id"]):
        ...
        for n,q in enumerate(INDEPENDENT[cid],1):
            expected = "International" if any(x in q.lower() for x in ("who", "world health organization", "lbm4")) else ("Malaysia" if any(x in q.lower() for x in ("malaysia", "malaysian", "form e", "gmm", "lmo")) else "Unspecified")
            rows.append({"case_id":f"C35-I-{cid}-{n}","case_type":"independent_claim","query":q,"acceptable_claim_ids":[cid],"expected_jurisdiction":expected})
    rows += [
      {"case_id":"C35-U-001","case_type":"unknown_evidence","query":"What exact Malaysian permit exemption applies to an unclassified future organism in a facility not yet described?","acceptable_claim_ids":[],"expected_jurisdiction":"Malaysia"},
      {"case_id":"C35-U-002","case_type":"unknown_evidence","query":"What is the current legal requirement for a jurisdiction not identified by the question?","acceptable_claim_ids":[],"expected_jurisdiction":"Unspecified"},
      {"case_id":"C35-U-003","case_type":"unknown_evidence","query":"Which reviewed source proves that this particular project is safe to start?","acceptable_claim_ids":[],"expected_jurisdiction":"Unspecified"},
      {"case_id":"C35-B-001","case_type":"boundary","query":"Is WHO laboratory biosafety guidance Malaysian law?","acceptable_claim_ids":["CLM-031","CLM-032","CLM-033"],"expected_jurisdiction":"International","required_boundary":"WHO_GUIDANCE_NOT_MALAYSIAN_LAW"},
      {"case_id":"C35-B-002","case_type":"boundary","query":"Does a clinical specimen transport document determine laboratory containment?","acceptable_claim_ids":["CLM-028"],"expected_jurisdiction":"Malaysia","required_boundary":"TRANSPORT_NOT_CONTAINMENT"},
      {"case_id":"C35-B-003","case_type":"boundary","query":"Does Form E itself approve the work?","acceptable_claim_ids":["CLM-025"],"expected_jurisdiction":"Malaysia","required_boundary":"FORM_E_IS_NOT_APPROVAL"},
      {"case_id":"C35-M-001","case_type":"minimal_pair","query":"How does WHO define biological risk?","acceptable_claim_ids":["CLM-038"],"expected_jurisdiction":"International"},
      {"case_id":"C35-M-002","case_type":"minimal_pair","query":"How should a Malaysian contained-use incident be reported?","acceptable_claim_ids":["CLM-009","CLM-017"],"expected_jurisdiction":"Malaysia"},
      {"case_id":"C35-M-003","case_type":"minimal_pair","query":"What packaging is used for a Malaysian clinical specimen?","acceptable_claim_ids":["CLM-026","CLM-027"],"expected_jurisdiction":"Malaysia"},
    ]
    return rows
...
class ProfileMixin:
    def classify(self, query: str):
        ...
        for p in _profiles():
            ...
            score=sum(term in q for term in p["terms"])
        ...
        if not matches:
            ...
            p=max(matches,key=lambda x:(x[1]["id"]=="WHO_RISK",x[0]))[1]
...
class PolicyMixin:
    def __init__(self, kb_path, manifest_path, policy_path, **kwargs):
        ...
        if self.policy.get("claim_use_status") != STATUS or self.policy.get("live_activation_status") != ACTIVATION:
            raise ValueError("C3.5 policy safety status invalid")
...
class C35CFG01(ProfileMixin, PolicyMixin, BioSafeCFG01): pass
class C35CFG02(ProfileMixin, PolicyMixin, BioSafeCFG02): pass
...
def _fixture(kb, policy):
    ...
    return out,pol,{"case_id":"C35-F-001","case_type":"new_document_fixture","query":"Which source describes a documented biosafety training record?","acceptable_claim_ids":[claim["claim_id"]],"expected_jurisdiction":"International"}

def _run(cls,kb,manifest,policy,rows):
    with tempfile.TemporaryDirectory() as td:
        p=Path(td); kp=p/"kb.json"; mp=p/"manifest.json"; pp=p/"policy.json"; kp.write_bytes(canonical_bytes(kb)); mp.write_bytes(canonical_bytes(manifest)); pp.write_bytes(canonical_bytes(policy)); r=cls(kp,mp,policy_path=pp) if hasattr(cls,"__init__") else cls(kp,mp); result=[]
        for c in rows:
            profile,hits=r.retrieve(c["query"],top_k=10); ids=[h["claim_id"] for h in hits]; accepted=bool(set(ids)&set(c["acceptable_claim_ids"])); unknown_ok=c["case_type"]!="unknown_evidence" or not hits; result.append({"case_id":c["case_id"],"case_type":c["case_type"],"ranked_claim_ids":ids,"query_profile":profile.__dict__,"first_acceptable_rank":next((ids.index(x)+1 for x in c["acceptable_claim_ids"] if x in ids),None),"recall_at_10":int(accepted),"route_ok":profile.jurisdiction==c["expected_jurisdiction"],"unknown_fail_closed":unknown_ok,"support_span_complete":all(h.get("support_spans") for h in hits if h["claim_id"] in c["acceptable_claim_ids"])})
        return result

def evaluate():
    kb,manifest,_=build_artifacts(); policy=build_artifact(); base=cases(kb); fkb,fpol,fc=_fixture(kb,policy); rows=base+[fc]; runs={}
    for name,cls in (("C35_METADATA_CFG01",C35CFG01),("C35_METADATA_CFG02",C35CFG02)):
        runs[name]={"metadata_on":_run(cls,fkb,manifest,fpol,rows),"metadata_off":_run(cls,fkb,manifest,fpol,rows)}
    summary={}
    for name,modes in runs.items():
        on=modes["metadata_on"]; ind=[x for x in on if x["case_type"]=="independent_claim"]; unk=[x for x in on if x["case_type"]=="unknown_evidence"]; fix=[x for x in on if x["case_type"]=="new_document_fixture"]; summary[name]={"independent_recall_at_10":sum(x["recall_at_10"] for x in ind)/len(ind),"independent_route_accuracy":sum(x["route_ok"] for x in ind)/len(ind),"unknown_fail_closed_rate":sum(x["unknown_fail_closed"] for x in unk)/len(unk),"new_document_recall_at_10":fix[0]["recall_at_10"],"hard_gates":{"canonical_independent_coverage":all(x["recall_at_10"] for x in ind),"unknown_fail_closed":all(x["unknown_fail_closed"] for x in unk),"fixture_onboarding":bool(fix[0]["recall_at_10"])}}
    return {"artifact_version":"BioSafe_PhaseC3_5_Independent_Generalization_Report_v0.1","benchmark_mode":"C3_5_INDEPENDENT_GENERALIZATION_GATE_REVIEW","gate_result":"BLOCKED_PENDING_OWNER_REVIEW","claim_use_status":STATUS,"live_activation_status":ACTIVATION,"case_count":len(rows),"input_hashes":{"query_profile_config":_hash(CONFIG),"fixture_claim":_hash(FIXTURES/"curated_claim_extension.json"),"fixture_source":_hash(FIXTURES/"source.txt")},"summary":summary,"cases":runs}
