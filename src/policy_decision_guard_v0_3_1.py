...
def _deterministic_refusal(reason: str) -> Dict[str, Any]:
    return {
        ...
        "applicable_authority": ["BioSafe safety policy"],
        "evidence": [],
        "missing_information": [],
...
def _add(constraints: List[str], *items: str) -> None:
...
def classify_policy(query: str) -> PolicyDecision:
    ...
    if (
        ...
        _add(
            ...
            "PROHIBITED conclusion: 'PPE makes the activity safe' or any equivalent claim.",
            "ALLOWED conclusion: 'PPE is one control layer; whether the activity is adequately controlled depends on the activity-specific risk assessment and other controls.'",
        )

    # General safety determination.
    if "PPE_LAYERED_CONTROL" not in matched and _match_any(ql, [
        ...
        _add(
            ...
            "PROHIBITED conclusion: 'The activity is safe/biosafe' or 'The activity is unsafe' when required facts are missing.",
            "ALLOWED conclusion: 'There is insufficient information to determine whether the activity is adequately controlled.'",
        )
    ...
    if _match_any(ql, [
        ...
        _add(
            ...
            "PROHIBITED conclusions: 'The project is compliant', 'The project is non-compliant', 'The project is approved', or equivalent certification language.",
            "ALLOWED conclusion: 'I cannot certify compliance or approval. Based on the supplied information, I can identify apparent alignment and gaps.'",
        )
    ...
    if "form e" in ql:
        if _match_any(ql, [
            ...
            _add(
                ...
                "PROHIBITED conclusions: 'Form E is legally required', 'Form E is not legally required', 'Your project legally requires Form E', unless the answer is explicitly quoting an authoritative determination already supplied.",
                "ALLOWED conclusion: 'Form E may be relevant if the activity falls within the applicable LMO contained-use/import-for-contained-use scope; the supplied facts are insufficient for an official legal determination.'",
            )
        ...
        if _match_any(ql, [
            ...
            _add(
                ...
                "PROHIBITED conclusion when any field is missing: 'No further information is needed', 'The form is complete', or equivalent.",
                "ALLOWED conclusion: 'The supported fields can be drafted, but the listed missing fields require confirmation.'",
            )
        ...
        if _match_any(ql, [
            ...
            _add(
                ...
                "PROHIBITED conclusion when information is missing: 'No inconsistencies found', 'The documents are fully consistent', or equivalent.",
                "ALLOWED conclusion: 'No inconsistency can be confirmed for the compared fields, but the listed missing information prevents a complete consistency determination.'",
            )
    ...
    if transport_terms:
        if _match_any(ql, [
            ...
            _add(
                ...
                "PROHIBITED conclusion: a bare 'Yes, it can be transported' or 'No, it cannot be transported' without the required shipment context.",
                "ALLOWED conclusion: 'A definitive transport determination cannot be made until the sample classification and shipment context are confirmed.'",
            )
        ...
        if "air" in ql:
            ...
            _add(
                ...
                "PROHIBITED conclusion: 'Road requirements apply universally to air transport' or 'No additional air requirements apply.'",
            )

    # Authority hierarchy: Malaysia vs WHO / institutional guidance.
    if (
        "who" in ql
        and _match_any(ql, [
        ...
        _add(
            ...
            "Do not automatically describe an institutional or IBC guideline as the Malaysian legal requirement.",
            "Do not say an institutional procedure categorically 'takes precedence' over Malaysian law/regulation.",
            "Distinguish: Malaysian legislation/regulations = legal requirements; official Malaysian guidance = authoritative guidance; institutional procedures = local requirements/procedures; WHO = international guidance.",
            "ALLOWED conclusion: 'For legal compliance, applicable Malaysian legislation/regulations govern. Institutional procedures may impose local requirements, while WHO provides international guidance. Escalate unresolved conflicts or ambiguity.'",
        )
    ...
    if _match_any(ql, [
        ...
        _add(
            constraints,
            "Do not assume an old online document remains current merely because it is official or still accessible.",
            "Verify official status, amendment, replacement or supersession and prefer current official sources.",
            "PROHIBITED conclusion before verification: 'The old guideline remains applicable/current.'",
            "ALLOWED conclusion: 'Its current status cannot be assumed; verify whether it has been amended, replaced or superseded.'",
        )
...
def build_policy_instruction(decision: PolicyDecision) -> str:
    ...
    lines += [
        "- Evaluate each proposition independently: a later caveat does not cancel an earlier prohibited conclusion.",
